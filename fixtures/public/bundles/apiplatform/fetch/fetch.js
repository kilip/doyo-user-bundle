...
function FetchStream(url, options) {
  ...
  this.responseBuffer = USE_ALLOC
    ? Buffer.alloc(0, "", "binary")
    : new Buffer(0, "binary");
  this.ended = false;
  this.readyToRead = 0;
}
...
FetchStream.prototype.drainBuffer = function() {
  ...
  if (this.responseBuffer.length > this.readyToRead) {
    push = USE_ALLOC
      ? Buffer.alloc(this.readyToRead, "", "binary")
      : new Buffer(this.readyToRead, "binary");
    this.responseBuffer.copy(push, 0, 0, this.readyToRead);
    restSize = this.responseBuffer.length - this.readyToRead;
    rest = USE_ALLOC
      ? Buffer.alloc(restSize, "", "binary")
      : new Buffer(restSize, "binary");
...
FetchStream.prototype.normalizeOptions = function() {
  ...
  if (
    !this.options.disableRedirect &&
    typeof this.options.maxRedirects !== "number" &&
  ...
  if (!this.options.method) {
    this.options.method =
      this.options.payload || this.options.payloadSize ? "POST" : "GET";
  }
...
FetchStream.prototype.parseUrl = function(url) {
  ...
  if (urlparts.auth) {
    var buf = USE_ALLOC
      ? Buffer.alloc(Buffer.byteLength(urlparts.auth), urlparts.auth)
      : new Buffer(urlparts.auth);
...
FetchStream.prototype.runStream = function(url) {
  ...
  if (this.options.payload) {
    url_data.urloptions.headers["content-length"] = Buffer.byteLength(
  ...
  if (this.options.asyncDnsLoookup) {
    var dnsCallback = function(err, addresses) {
      ...
      url_data.urloptions.headers.host =
        url_data.urloptions.hostname || url_data.urloptions.host;
      url_data.urloptions.hostname = addresses[0];
      url_data.urloptions.host =
        url_data.urloptions.headers.host +
        (url_data.urloptions.port ? ":" + url_data.urloptions.port : "");
  ...
  var req = url_data.transport.request(
    ...
    function(res) {
      ...
      if ([301, 302, 303, 307, 308].indexOf(res.statusCode) >= 0) {
        if (
          !this.options.disableRedirects &&
          this.options.maxRedirects > this._redirect_count &&
        ...
        receive = function(chunk) {
          ...
          if (maxlen >= chunk.length) {
            ...
            this.responseBuffer = Buffer.concat(
              [this.responseBuffer, chunk],
              this.responseBuffer.length + maxlen
            );
          }
          this.drainBuffer();
        }.bind(this),
  ...
  fetchstream.on("end", function() {
    ...
    if (options.outputEncoding) {
      return callback(
        null,
        response_data,
        buffer.toString(options.outputEncoding)
      );
    } else {
